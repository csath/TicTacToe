...
export const isDraw = (cellState: CellOccupancy[]): boolean => {
  return cellState.every(
    (cell) =>
      cell === CellOccupancy.PLAYER_ONE || cell === CellOccupancy.PLAYER_TWO
  );
};
...
export const isWin = (
  ...
  const currentPlayerMoves = cellState
    .map((state, index) => (state === currentPlayer ? index : null))
    .filter((item: any) => item != null) as number[];
  return winningCombinations.some((combination) =>
    combination.every((index) => currentPlayerMoves.includes(index))
  );
};

export const generateComputerPlayResult = (
  cellState: CellOccupancy[]
): number => {
  const possibleIndexes = cellState
    .map((state, index) => (state === CellOccupancy.NONE ? index : null))
    .filter((item: any) => item != null) as number[];
  return getRandomItemFromArray(possibleIndexes);
};

export const getGameBoardHeaderText = (
